...
static bool atm90e26_stub_read_reg(uint16_t reg, uint16_t *value)
{
    (void)reg;
...
static bool atm90e26_stub_write_reg(uint16_t reg, uint16_t value)
{
    (void)reg;
    (void)value;
    return true;
...
{
    app->snapshot = *atm90e26_get_measurement(&app->meter);
}
...
{
    atm90e26_init(&app->meter, atm90e26_stub_read_reg, atm90e26_stub_write_reg);
    (void)atm90e26_configure(&app->meter);
...
void device_app_process(device_app_t *app, uint32_t tick_ms)
{
    ...
    if ((tick_ms - app->last_measurement_update_ms) >= APP_MEASUREMENT_UPDATE_MS)
    {
        ...
        (void)atm90e26_read_measurement(&app->meter);
        ...
        else if (protection_check_overvoltage(&app->snapshot))
        {
            relay_control_set(&app->relay, RELAY_CMD_OFF, tick_ms);
